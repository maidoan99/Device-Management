                ...
                <div class="login-panel panel">
                    <div class="panel-heading row justify-content-md-center">
                        <div class="col-7 panel-heading-item text-center">
                            <h4 class="panel-title">Thêm yêu cầu</h4>
                        </div>
                    </div>
                    ...
                    <div class="panel-body">
                        ...
                        <form id="form-submit" action="{{route('request.store')}}" method="POST" enctype="multipart/form-data">
                            <!-- de truyen du lieu -->
                            {{ csrf_field() }}
                            <div class="form-group">
                                <label><b>Lí do <span class="obligatory"> (*)</span></b></label>
                                <textarea name="reason" rows="5" class="w-100"></textarea>
                            </div>
                            @if (count($errors) > 0)
                            <div class="error-message">{{ $errors->first('reason') }}</div>
                            @endif

                            <button class="btn btn-lg btn-secondary mt-3"><a href="{{ route('request.me.index') }}">Back</a></button>
